...
function HeaderDropdown({
  ...
  return (
    <div className="relative group flex items-center h-full cursor-pointer">
      <Link
        href={href || "#"}
        className="inline-flex items-center justify-center gap-x-1 text-sm font-bold tracking-tight text-white group-hover:text-brand-secondary h-full py-4">
        {label}
        <ChevronDownIcon
      ...
      <div className="absolute top-[calc(100%-8px)] left-0 z-50 w-72 bg-white shadow-xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300">
        <div className="p-6 flex flex-col">
          ...
          <h3 className="text-brand-secondary text-[18px] font-bold mb-6">
            {label}
          </h3>
    ...
    <div className="w-full">
      <button
        ...
        className={`w-full flex items-center justify-between px-6 py-4 text-sm font-bold transition-colors ${
          isOpen
            ? "bg-brand-dark text-[#F2EDE7]"
            : "text-white border-b border-brand-dark/20 hover:bg-brand-dark/5"
        }`}>
        {label}
        <ChevronDownIcon
          className={`h-5 w-5 transition-transform ${isOpen ? "rotate-180 text-[#F2EDE7]" : ""}`}
        />
      </button>
      {isOpen && (
        <div className="flex flex-col bg-brand-dark/30">
          {items.map((item) => (
            <Link
              key={item.label}
              href={item.href}
              onClick={onClose}
              className="flex items-center gap-3 px-8 py-4 border-b border-brand-dark/15 group">
              <ChevronRightIcon className="h-4 w-4 text-brand-dark-border group-hover:translate-x-1 transition-transform" />
              <span className="text-white text-xs font-bold group-hover:text-brand-secondary transition-colors">
                {item.label}
              </span>
            </Link>
          ))}
...
export default function MainHeader() {
  ...
  return (
    <header className="fixed inset-x-0 top-0 z-40 flex h-16 md:h-18 items-center justify-between px-4 md:px-10 text-white bg-brand-dark-border transition-all duration-300">
      {/* Logo */}
      <div className="shrink-0 pt-2 z-50">
        <Link href="/" className="flex items-center" aria-label="Lucru home">
          <Image
            src={logo}
            alt="AskTalentRecruitment"
            priority
      ...
      <div className="md:hidden flex items-center pt-2 z-50 cursor-pointer">
        <button
          onClick={() => setIsMobileMenuOpen(true)}
          className="text-white hover:text-brand-secondary focus:outline-none"
      ...
      <div className="hidden md:flex items-center space-x-6 h-full">
        ...
        <nav className="flex items-center text-brand-primary space-x-6 ">
          {SHOW_FULL_NAV && (
            <>
              <HeaderDropdown
                label="Hire Manpower"
          ...
          <Link
            ...
            className="font-bold text-sm tracking-tight text-white hover:text-brand-primary">
            Success Stories
          </Link>
          <Link
            href="/vacancies"
      ...
      <Transition
        ...
        leaveTo="opacity-0">
        <div className="md:hidden fixed inset-0 z-50 bg-brand-dark overflow-y-auto w-full min-h-screen text-white">
          ...
          <div className="flex flex-col w-full text-white pb-10">
            ...
            <Link
              href="/success-stories"
              className="px-6 py-4 text-[16px] font-bold text-white border-b border-white/20 w-full text-left hover:bg-white/5 hover:text-brand-secondary transition-colors"
              onClick={() => setIsMobileMenuOpen(false)}>
              Success Stories
            </Link>
            <Link
